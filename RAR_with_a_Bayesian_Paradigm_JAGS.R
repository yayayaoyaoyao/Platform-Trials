...
snap_fun <- function(datapop=sim11,jj=1,x,N2=7500,aaa,sss,mmm,intercept1,interaction1  ) {
  ...
  assign1<-blockrand(blocksize=30*2*2,N2,armn=30*2,armlabel=levellab)
  armleft<-levellab
  ...
  for (jjj in 1:15){ #1:74 74 because ended when the total sample size is 7300
   print(jjj)
    if (jjj==1){
      stopnum=500#200
    }else{
      stopnum=500*jjj#200+100*(jjj-1)
    }
    ...
    data11a<-data1[complete.cases(data1),  ][1:stopnum,]
...
"
  ...
  y = as.numeric(data11a[,10])
  xA = as.numeric(data11a[,7])
  xS = as.numeric(data11a[,8])
  xM = as.numeric(data11a[,9])
  xAS = as.numeric(data11a[,6])#shock
  Ntotal = nrow( data11a)
  ...
  mcmc_samples1 <- coda.samples(model, parameters, n.iter=5000)#25000
  mcmc_samples1 <-as.data.frame(as.matrix(mcmc_samples1))
  mcmc_samples1<- mcmc_samples1[,c('aS[1]','aS[2]','bA[1]','bA[2]','bA[3]','bA[4]','bS[2]','bS[3]',
                                     'bM[2]', 'aSBS[2,2]','aSBS[2,3]')]

  mcmc_samples_var<- mcmc_samples1 %>% mutate(shock_s2= mcmc_samples1$'bS[2]'+ mcmc_samples1$'aSBS[2,2]',
                                              shock_s3=mcmc_samples1$'bS[3]'+mcmc_samples1$'aSBS[2,3]') %>%summarise_if(is.numeric, var)%>%
      as.data.frame()
